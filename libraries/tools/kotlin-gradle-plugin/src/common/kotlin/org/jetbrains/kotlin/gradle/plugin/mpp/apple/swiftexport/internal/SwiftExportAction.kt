...
internal abstract class SwiftExportAction : WorkAction<SwiftExportAction.SwiftExportWorkParameters> {
    ...
    override fun execute() {
        ...
        val exportModules = parameters.swiftModules.zip(exportSettings) { modules, settings ->
            modules.map { module ->
                module.toInputModule(createModuleConfig(module.flattenPackage, module.isTransitive, settings))
            }.toSet()
        }.get()
    ...
    private fun createModuleConfig(flattenPackage: String?, transitive: Boolean, settings: Map<String, String>): SwiftModuleConfig {
        return SwiftModuleConfig(
            ...
            shouldBeFullyExported = !transitive,
        )
    }
