...
class SwiftExportUnitTests {
    ...
    fun `test swift export exported modules`() {
        ...
        val expectedModules = buildSmartList {
            add(SwiftExportModuleForAssertion("Subproject", "subproject-iosSimulatorArm64Main.klib", false))
    ...
    fun `test non-exported compilation dependencies are not exported`() {
        ...
        val swiftExportTask = project.tasks.withType(SwiftExportTask::class.java).single()
        val actualModules = swiftExportTask.parameters.swiftModules.getOrElse(emptyList()).filter { !it.isTransitive }

        val expectedModules = buildSmartList {
            add(SwiftExportModuleForAssertion("Decompose", "decompose.klib", false))
        }
    ...
    fun `test dependency export custom parameters`() {
        ...
        val expectedModules = buildSmartList {
            add(SwiftExportModuleForAssertion("CustomDecompose", "decompose.klib", false))
        }
    ...
    fun `test exporting transitive dependencies`() {
        ...
        val expectedModules = buildSmartList {
            ...
            add(SwiftExportModuleForAssertion("Atomicfu", "atomicfu.klib", true))
        }
...
private fun List<SwiftExportedModule>.toModulesForAssertion() = map {
    SwiftExportModuleForAssertion(it.moduleName, it.artifact.name, it.isTransitive)
}
