 ...
 */
internal class NegativeCaffeineCache<K : Any, V : Any>(configure: (Caffeine<Any, Any>) -> Caffeine<Any, Any>) {
     ...
     */
    fun getOrCompute(key: K, compute: (K) -> V?): V? = cache.get(key) { compute(it) ?: NullValue }?.nullValueToNull()
}
