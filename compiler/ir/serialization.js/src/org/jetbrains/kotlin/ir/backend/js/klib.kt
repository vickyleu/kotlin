...
fun loadIrForMultimoduleMode(depsDescriptors: ModulesStructure, irFactory: IrFactory): IrModuleInfo {
    val mainModule = depsDescriptors.mainModule
    val configuration = depsDescriptors.compilerConfiguration
    val allDependencies = depsDescriptors.allDependencies
    ...
    val deserializedStdlib = irLinker.deserializeHeadersWithInlineBodies(
    ...
    sortedDependencies.forEach { klib ->
        ...
        if (stdlibDependency != klib && klib != mainDependency) {
    ...
    val deserializedMain = irLinker.deserializeIrModuleHeader(
        depsDescriptors.getModuleDescriptor(mainDependency), mainDependency, { DeserializationStrategy.ALL }
    )
