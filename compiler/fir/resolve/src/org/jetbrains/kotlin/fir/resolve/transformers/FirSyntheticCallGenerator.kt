...
class FirSyntheticCallGenerator(
    ...
    fun resolveAnonymousFunctionExpressionWithSyntheticOuterCall(
        ...
        (resultingCall.calleeReference as? FirResolvedErrorReference)?.let {
            ...
            diagnostic.actualTypeIfMismatch(anonymousFunctionExpression, expectedTypeData)

            if ((diagnostic as? ConeInapplicableCandidateError)?.candidate?.diagnostics?.isEmpty() != true) {
    ...
    private fun ConeDiagnostic.actualTypeIfMismatch(
        ...
        if (expectedTypeData?.expectedTypeMismatchIsReportedInChecker != true) return
        if (this !is ConeInapplicableCandidateError) return
        val argumentTypeMismatchOnWholeLambda = candidate.diagnostics.singleOrNull {
            it is ArgumentTypeMismatch && it.argument == anonymousFunctionExpression
        ...
        candidate.removeDiagnostic(argumentTypeMismatchOnWholeLambda)
    }
