...
fun FirFunctionCall.replaceLambdaArgumentEffects(session: FirSession) {
    ...
    effects?.forEach { fir ->
        when (val effect = fir.effect) {
            ...
            is ConeConditionalEffectDeclaration -> (effect.effect as? ConeInsideLambdaEffectDeclaration)?.let { insideEffect ->
                // TODO: error reporting
                val conditionParameterIndex = (effect.condition as? KtBooleanValueParameterReference)?.parameterIndex
                if (conditionParameterIndex != null) {
                    val conditionParameter = function.valueParameters.getOrNull(conditionParameterIndex)
                    val lambdaParameter = function.valueParameters.getOrNull(insideEffect.valueParameterReference.parameterIndex)
    ...
    for ((argument, parameter) in argumentMapping) {
        ...
        eventOccurencesRangeByParameter[parameter] ?: EventOccurrencesRange.UNKNOWN.takeIf {
        ...
        conditionParameterByParameter[parameter]?.let { conditionParameter ->
            ...
            if (conditionExpression != null) {
                lambda.lambdaArgumentInsideTruths = lambda.lambdaArgumentInsideTruths.orEmpty() + conditionExpression
            }
        }
    }
}
